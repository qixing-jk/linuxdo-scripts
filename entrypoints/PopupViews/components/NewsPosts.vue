	...
	<ul class="list" v-if="this.list.length > 0">
		<li v-for="item in list" :key="item.id" class="news-item">
			...
			<div class="news-content">
				<a :href="'https://linux.do/t/topic/' + item.id" @click="handleLinkClick($event, item.id)" class="news-link">
					{{ formatTitle(item.title) }}
				</a>
			</div>
			<div class="news-meta">
				<!-- 当 isReadonlyBefore 为 false 时，将按钮放在右侧 -->
				<button v-if="!isReadonlyBefore" class="preview-btn" @click="previewPost(item.id)" title="设为已读">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						width="16"
						height="16"
...
export default {
	...
	beforeDestroy() {
		...
		try {
			...
			window.removeEventListener('readonlyBeforeChanged', this.handleReadonlyBeforeChanged);
		} catch (error) {
